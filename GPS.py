...
def findSpeed(df, time_interval=5, speed_diff_pct=0.5):
    computeDistance(df)
    ...
    for N in range(len(df)):
        ...
        average_speed = df.loc[:N, 'cspeed'].mean()

        dynamic_threshold = average_speed * (1 + speed_diff_pct)

        if abs(speed_diff) > dynamic_threshold:
